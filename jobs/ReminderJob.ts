...
export class ReminderJob {
	private jobName = JobId.REMINDERS_JOB;
	constructor(private app: RemindApp) {}

	public getReminderJob(): IProcessor {
	...
	public async scheduleReminder(
		modify: ISchedulerModify,
		reminderId: string,
		when: Date,
	): Promise<string> {
		const jobContext: IReminderJobContext = {
			reminderId,
		};

		const jobId = await modify.scheduleOnce({
			id: this.jobName,
			when,
			data: jobContext,
		});

		// TODO: there's a chance jobId is undefined here due to some bug in previous RC version.
		// however since we don't need jobId anyway, we can just ignore this for now.
		return jobId || '';
	}
